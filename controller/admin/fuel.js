...
module.exports = function () {
  ...
  /**
   * @route POST /fuel/delete
   * @desc Delete Fuel Record
   */
  controller.deleteFuel = async function (req, res) {
    try {
      const { id } = req.body;
      if (!id) return res.send({ status: false, message: "Fuel ID required" });

      const result = await db.DeleteDocument("fuel", { _id: new mongoose.Types.ObjectId(id) });

      if (result.status) {
        return res.send({ status: true, message: "Fuel record deleted successfully" });
      } else {
        return res.send({ status: false, message: "Failed to delete record" });
      }
    } catch (err) {
      console.log("ERROR deleteFuel", err);
      return res.send({ status: false, message: "Error deleting fuel record" });
    }
  };
