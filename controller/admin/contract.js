...
module.exports = function () {
  ...
  try {
    ...
    let contractData = {
      ...
      // ✅ Convert to Number
      busesDeployed: Number(body.busesDeployed) || 0,
      driversDeployed: Number(body.driversDeployed) || 0,

      // ✅ Support multiple types
      contractType: Array.isArray(body.contractType)
        ? body.contractType
    ...
    let result;
    if (body._id) {
      result = await db.UpdateDocument("contract", { _id: new mongoose.Types.ObjectId(body._id) }, contractData);
      return res.send({ status: true, message: "Contract updated successfully", data: result });
    } else {
      result = await db.InsertDocument("contract", contractData);
      return res.send({ status: true, message: "Contract added successfully", data: result });
    }

  } catch (error) {
    console.log(error, "ERROR saveContract");
    return res.send({ status: false, message: "Something went wrong while saving contract." });
  }
};
 ...
 controller.listContracts = async function (req, res) {
  try {
    ...
    if (status !== undefined) match.status = parseInt(status);
    if (contractType) match.contractType = { $in: contractType.split(",").map(x => x.trim()) };
    ...
    let result = await db.GetAggregation("contract", [
      { $match: match },
      {
        $project: {
          clientName: 1,
    ...
    return res.send({ status: true, count: result.length, data: result });
  } catch (error) {
    console.log(error, "ERROR listContracts");
    return res.send({ status: false, message: "Something went wrong while fetching contracts." });
  }
};
