...
module.exports = function () {
  ...
  controller.saveAttendance = async function (req, res) {
    try {
      ...
      let existingAttendance = await db.GetOneDocument(
        "attendance",
        { employee: employeeId },
        {},
        {}
      ...
      if (recordExists) {
        ...
        await db.UpdateDocument(
          "attendance",
          { employee: employeeId },
          { $push: { records: { date: targetDate, status, remarks } } }
        );
        return res.send({
          status: true,
 ...
 controller.saveCustomerPayment = async function (req, res) {
  ...
  controller.listCustomerPayments = async function (req, res) {
    try {
      ...
      if (startDate && endDate)
        match.dueDate = { $gte: new Date(startDate), $lte: new Date(endDate) };
      ...
      const [data, totalA] = await Promise.all([
        ...
        db.GetAggregation("customerPayment", [
          { $match: match },
          { $count: "total" },
        ]),
      ]);
      ...
      return res.send({
        status: false,
        message: "Error while listing customer payments",
      });
    }
  };
  ...
  controller.listVendorPayments = async function (req, res) {
    try {
      ...
      const [data, totalA] = await Promise.all([
        ...
        db.GetAggregation("vendorPayment", [
          { $match: match },
          { $count: "total" },
        ]),
      ]);
      ...
      return res.send({
        status: false,
        message: "Error while listing vendor payments",
      });
    }
  };
